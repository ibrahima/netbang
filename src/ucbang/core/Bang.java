...
public class Bang {
	...
	/*
	 * public static void main(String[] args){ new Bang(); }
	 */
	Server server;
	...
	public ArrayList<Card> drawPile = new ArrayList<Card>(); // the card on the
																// bottom in
																// stored in
																// index 0, the
																// card on top
	...
	/**
	 * Create p players. Create a draw pile. Show everyone their roles. Give
	 * them a choice between two character cards. Give sheriff the first turn.
	 * Draw cards equal to the number of life points. Sheriff gets an additional
	 * card.
	 * 
	 * @param p
	 */
	public void start(int p) {
		...
		for (int n = 0; n < numPlayers; n++) {
			int role = roles.remove((int) (Math.random() * roles.size()))
					.ordinal();
		...
		for (int n = 0; n < numPlayers; n++) {
			drawPile.add(new Card(charList
					.remove((int) (Math.random() * charList.size()))));
			drawPile.add(new Card(charList
					.remove((int) (Math.random() * charList.size()))));
		...
		/*
		 * for(int n = 0; n<numPlayers; n++){ //doesn't prompt all players at
		 * the same time System.out.println("1. " + players[n].hand.get(0).name
		 * + " HP: " + players[n].hand.get(0).special); System.out.println("2. "
		 * + players[n].hand.get(1).name + " HP: " +
		 * players[n].hand.get(1).special); Card c =
		 * players[n].hand.get(gui[n].promptChooseCharacter(players[n].hand));
		 * players[n].character = c.ordinal; players[n].maxLifePoints +=
		 * c.special; //special is hp for char cards players[n].lifePoints =
		 * players[n].maxLifePoints; playerDiscardHand(players[n]);
		 * gui[n].paint(gui[n].getGraphics()); //TODO: this shouldn't here, but
		 * this is the only place where it didn't glitch up }
		 */
		// System.out.println("asdfasdfasdfasfasdfasfasfasfasdf1");
		while (server.prompting > 0) {
			try {
				Thread.sleep(100);
			} catch (Exception e) {
			}
		...
		while (allCards.size() > 0) {
			drawPile.add(new Card(allCards
					.remove((int) (Math.random() * allCards.size()))));
		}
		...
		gui[0].appendText("\nYou are: "
				+ Characters.values()[players[0].character] + ", the "
				+ players[0].role.name() + "\n");
	...
	public boolean nextTurn() {
		...
		// draw two cards
		if (players[turn % numPlayers].specialDraw == 0) { // TODO: get rid of
															// specialDraw, move
															// to a direct
															// reference to
															// character cards
		...
		while (card != -1) {
			if (players[turn % numPlayers].hand.size() > 0) {
				if (isGameWon())
					return false;
				card = gui[turn % numPlayers].promptChooseCard(players[turn
						% numPlayers].hand, "Play a card!", "It's your turn",
						false);
				if (card != -1)
					playCardFromHand(players[turn % numPlayers], players[turn
							% numPlayers].hand.get(card));
	...
	/**
	 * Plays a card. This is one of the functions used to connect the GUI to the
	 * game.
	 * 
	 * @param c
	...
	public boolean playCardFromHand(Player p, Card c) {
		...
		if (c.type == 2) {
			...
			if (c.target == 2) {
				...
				targets = new int[1]; // serves no purpose but to initialize
										// value
			}
			...
			if (c.effect == Card.play.DAMAGE.ordinal()) {
				for (int target : targets) {
					...
					while (miss != -1
							|| (miss >= 0
									&& miss < players[target].hand.size()
									&& players[target].hand.get(miss).special == (c.name == CardName.INDIANS
											.name() ? 1 : 0) && players[target].hand
									.get(miss).effect == (c.name == CardName.INDIANS
									.name() ? Card.play.DAMAGE.ordinal()
									: Card.play.DAMAGE.ordinal()))) {
						miss = gui[target].promptChooseCard(
								players[target].hand, "Dodge!", "Play a miss?",
								false);
					}
					if (miss == -1) { // change this to a flag checking
										// barrels/if target want to play a
										// miss, etc.
						...
						System.out.println(target + "'s hp: "
								+ players[target].lifePoints);
						if (players[target].lifePoints <= 0) {
							gui[p.id].appendText("You killed player " + target
									+ "! \nPlayer " + target + " was a(n) "
									+ players[target].role.name() + " ("
									+ players[target].role.ordinal() + ")");
							if (players[target].role.ordinal() == 2) { // if he
																		// was
																		// an
																		// outlaw
																		// ,
		...
		// TODO: currently only removes the card from hand and sets it into
		// discard
		return true;
	}
	...
	public void playerDrawCard(Player p, int n) {
		/*
		 * if(n <= 2) gui[p.id].appendText("Draw "+n+" card(s)."); else if(n>2)
	...
	/**
	 * Shuffles the discard pile back into the deck. Only used when draw pile is
	 * empty
	 */
	public void shuffleDeck() {
		...
		while (discardPile.size() > 0) {
			drawPile.add(discardPile.remove((int) Math.random()
					* discardPile.size()));
		}
	}
}
